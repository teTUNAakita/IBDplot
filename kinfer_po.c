/*--------------------------------------------------------
 Mar.3, 2020
 Simulate genotype data of PO for "SNPRelate"
 ...
 [Operation]
 gcc -DHAVE_INLINE -lgsl -lm -lgslcblas  -Wall -o kinfer_po kinfer_po.c
./kinfer_po [maker_number] [error_rate]
 --------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main( int argc, char *argv[] ) {
  ...
  for ( int i = 0; i < maker_number; i++ ) {
    ...
    if (debug) printf("p1: %d%d\n", allele_p1[ 0 ],allele_p1[ 1 ]) ;
    ...
    if ( (allele_p1[ 0 ] == 0)&&(allele_p1[ 1 ] == 0) ) {
      G1 = 0 ;
    } else if ( (allele_p1[ 0 ] == 1)&&(allele_p1[ 1 ] == 1) ) {
      G1 = 2 ;
    } else {
      G1 = 1 ;
    }
...
//------------------------------------------------------------------------------------------
int usage ( ) {
  fprintf ( stderr, "usage: kinfer_po maker_number error_rate\n" );
  exit ( 1 ) ;
}
